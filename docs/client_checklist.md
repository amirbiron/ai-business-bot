...
| 13 | טון תקשורת (ראו אפשרויות למטה) | `SYSTEM_PROMPT` (config.py), כלל 4 | friendly |

### אפשרויות טון תקשורת

הלקוח בוחר אחד מארבעת הטונים הבאים. לאחר הבחירה — יש לעדכן את **כלל 4** ב-`SYSTEM_PROMPT` שב-`config.py` בהתאם.
