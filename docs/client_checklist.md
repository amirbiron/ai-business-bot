...
| 13 | סגנון תקשורת מועדף (פורמלי/ידידותי) | `SYSTEM_PROMPT` (config.py) | חם, מועיל ותמציתי |
...
- [ ] התאמת `SYSTEM_PROMPT` — סגנון התקשורת, כללי התנהגות ספציפיים ללקוח

### 4.3 התאמת הודעות הבוט (`bot/handlers.py`)

- [ ] הודעת `/start` — ברכת פתיחה בהתאם לעסק
- [ ] טקסט כפתורים (מחירון, קביעת תור, מיקום, נציג)
