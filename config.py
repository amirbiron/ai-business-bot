...
# ארבעה טונים מוכנים מראש — המפתח נשמר ב-DB ומוזרק לכלל 4 ב-system prompt
BOT_TONE_PRESETS: dict[str, tuple[str, str]] = {
    "friendly": (
        "ידידותי",
        "היה חם, מועיל ותמציתי. השתמש בטון שיחתי. "
        "אפשר להשתמש באמוג'י מדי פעם (😊, ✨, 👋).",
    ),
    "formal": (
        "רשמי",
        "השתמש בשפה מקצועית ואדיבה. ללא סלנג, קיצורים או אמוג'י. "
        "שמור על נימוס גבוה ומקצועיות.",
    ),
    "sales": (
        "מכירתי",
        "השתמש בטון אנרגטי וממוקד פעולה. הנחה את הלקוח לצעד הבא בצורה טבעית "
        "(קביעת תור, רכישה, ניסיון). הדגש ערך ויתרונות.",
    ),
    "luxury": (
        "יוקרתי",
        "השתמש בשפה אלגנטית ומינימליסטית. צור תחושת VIP ובלעדיות. "
        "ללא אמוג'י. כל מילה צריכה לשדר איכות ומקצועיות.",
    ),
}

# ברירת מחדל מ-ENV (ניתן לדריסה דרך פאנל האדמין)
BOT_TONE_DEFAULT = os.getenv("BOT_TONE", "friendly")
BOT_CUSTOM_INSTRUCTIONS = os.getenv("BOT_CUSTOM_INSTRUCTIONS", "")
